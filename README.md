...
Por mais que tenham sido constatados valores nulos da camada Bronze, especialmente na tabela **ons_balanco_energia_subsistema**, que posteriormente originou as tabelas **balanco_energia** e **balanco_energia_sin** na camada Silver, esses valores nulos significam a realidade da operação e não uma anomalia: valores de geração solar, por exemplo, nula ou muito próximas de zero são aceitáveis, visto que tratando-se do contexto é uma fonte de energia que deixa de produzir em períodos noturnos. O mesmo pode ocorrer para outras fontes que não são acionadas em determinados momentos. Abaixo, uma amostra para exemplificar este fato:
